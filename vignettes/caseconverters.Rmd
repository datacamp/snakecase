...
If you see unwanted underscores around specific pattern, which you don't want to delete
via `preprocess`, just use `protect`:
...
`to_any_case()` is vectorised over `string`, `preprocess`, `postprocess`, `prefix`,
`postfix` and `protect`.
